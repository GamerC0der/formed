        ...
        <div key={formKey} className="space-y-6">
          {formData.formComponents.map((component: any) => (
            <div key={component.id} className="space-y-2">
              ...
              {component.type === 'radio' && <div className="space-y-3">{component.options?.map((option: string, i: number) => <div key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-800/50 transition-colors"><input type="radio" name={component.id} className="w-4 h-4 text-blue-600 bg-gray-800 border-gray-500 focus:ring-blue-500" /><span className="text-gray-300 text-sm">{option}</span></div>)}</div>}
